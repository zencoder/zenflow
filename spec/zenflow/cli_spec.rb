...
describe Zenflow::CLI do
  ...
  describe "#already_configured" do
    ...
    context "when the user wants to overwrite the configuration" do
      before do
        expect(Zenflow).to receive(:Ask).with(*question).and_return('y')
      end
    ...
    context "when the user does not want to overwrite the configuration" do
      before do
        expect(Zenflow).to receive(:Ask).with(*question).and_return('n')
      end

      it "aborts" do
        expect(Zenflow).to receive(:Log).with('Aborting...', :color => :red)
        expect(lambda{ subject.already_configured}).to raise_error(SystemExit)
      end
    end
  end
  ...
  describe "#configure_branch" do
    context "when the user wants to configure a staging branch" do
      before do
        expect(Zenflow).to receive(:Ask).with("Use a branch for staging releases and hotfixes?", :options => ["Y", "n"], :default => "y").and_return('y')
      end

      it 'names the staging branch whatever the user wants' do
        expect(Zenflow).to receive(:Ask).with("What is the name of that branch?", :default => "staging").and_return('staging')
    ...
    context "when the user does not want to configure a staging branch" do
      before do
        expect(Zenflow).to receive(:Ask).with("Use a branch for staging releases and hotfixes?", :options => ["Y", "n"], :default => "y").and_return('n')
      end
  ...
  describe "#configure_merge_strategy" do
    context "when the user wants to keep the default merge strategy of 'merge'" do
      it 'sets the merge strategy to merge' do
        expect(Zenflow).to receive(:Ask).with("What merge strategy would you prefer?", :options => ["merge", "rebase"], :default => "merge").and_return('merge')
    ...
    context "when the user wants to change the default merge strategy to 'rebase'" do
      it 'sets the merge strategy to rebase' do
        expect(Zenflow).to receive(:Ask).with("What merge strategy would you prefer?", :options => ["merge", "rebase"], :default => "merge").and_return('rebase')
  ...
  describe "#configure_project" do
    it 'asks the user to name their project' do
      expect(Zenflow).to receive(:Ask).with("What is the name of this project?", :required => true).and_return('zenflow')
  ...
  describe "#configure_branches" do
    it 'configures branches for the project' do
      expect(Zenflow).to receive(:Ask).with("What is the name of the main development branch?", :default => "master").and_return('master')
  ...
  describe "#configure_remotes" do
    context "when the user wants to configure a backup remote" do
      before do
        expect(Zenflow).to receive(:Ask).with("Use a backup remote?", :options => ["y", "N"], :default => "n").and_return('y')
      end

      it 'configures the primary remote and a backup remote' do
        expect(Zenflow).to receive(:Ask).with("What is the name of your primary remote?", :default => "origin").and_return('origin')
        expect(Zenflow::Config).to receive(:[]=).with(:remote, 'origin')
        expect(Zenflow).to receive(:Ask).with("What is the name of your backup remote?", :default => "backup").and_return('backup')
    ...
    context "when the user does not want to configure a backup remote" do
      before do
        expect(Zenflow).to receive(:Ask).with("Use a backup remote?", :options => ["y", "N"], :default => "n").and_return('n')
      end

      it 'configures the primary remote and a backup remote' do
        expect(Zenflow).to receive(:Ask).with("What is the name of your primary remote?", :default => "origin").and_return('origin')
        expect(Zenflow::Config).to receive(:[]=).with(:remote, 'origin')
        expect(Zenflow).to_not receive(:Ask).with("What is the name of your backup remote?", :default => "backup")
  ...
  describe "#set_up_changelog" do
    context "when the changelog doesn't already exist" do
      ...
      context "when the user wants to set up a changelog" do
        it 'sets up the changelog' do
          expect(Zenflow).to receive(:Ask).with("Set up a changelog?", :options => ["Y", "n"], :default => "y").and_return('y')
      ...
      context "when the user does not want to set up a changelog" do
        it 'does not set up the changelog' do
          expect(Zenflow).to receive(:Ask).with("Set up a changelog?", :options => ["Y", "n"], :default => "y").and_return('n')
    ...
    context "when the changelog already exists" do
      ...
      it 'does not set up the changelog' do
        ...
        expect(Zenflow).to_not receive(:Ask).with("Set up a changelog?", :options => ["Y", "n"], :default => "y")
        expect(Zenflow::Changelog).to_not receive(:create)
        subject.set_up_changelog
      end
    end
  ...
  describe "#confirm_some_stuff" do
    it "confirms staging deployment and code review requirements" do
      ...
      expect(Zenflow).to receive(:Ask).with("Require deployment to a staging environment?", :options => ["Y", "n"], :default => "y").and_return('y')
      expect(Zenflow::Config).to receive(:[]=).with(:confirm_staging, true)
      expect(Zenflow).to receive(:Ask).with("Require code reviews?", :options => ["Y", "n"], :default => "y").and_return('n')
  ...
  describe "#init" do
    ...
    context "when zenflow has already been configured" do
      ...
      context 'and it is forced to initialize' do
        before do
          expect(Zenflow).to receive(:Log).with('Warning', :color => :red)
          expect(Zenflow).to receive(:Ask).and_return('n')
          expect(Zenflow).to receive(:Log).with('Aborting...', :color => :red)
        end
        ...
        it 'calls already_configured' do
          ...
          expect(lambda{ subject.init}).to raise_error(SystemExit)
        end
      end
    end
  end
