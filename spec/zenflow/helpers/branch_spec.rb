...
describe Zenflow::Branch do
  describe 'self.list' do
    context "when the prefix has no branches" do
      before do
        expect(Zenflow::Shell).to receive(:run).with("git branch | grep features", :silent => true).and_return("")
      end
  ...
  describe "self.current" do
    context "when the current branch doesn't match the prefix" do
      before do
        expect(Zenflow::Shell).to receive(:run).with("git branch | grep '* feature'", :silent => true).and_return("")
      end
  ...
  describe "self.create" do
    it "updates the branch" do
      expect(Zenflow).to receive(:Log).with("Creating the feature/test-branch-creation branch based on master")
  ...
  describe "self.track" do
    ...
    context "when a remote is configured" do
      ...
      it "tracks the branch in that remote" do
        expect(Zenflow).to receive(:Log).with("Tracking the feature/test-tracking branch against some-remote/feature/test-tracking")
        expect(Zenflow::Shell).to receive(:run).with("git branch --set-upstream-to=some-remote/feature/test-tracking feature/test-tracking")
    ...
    context "when a remote is not configured" do
      ...
      it "tracks the branch in origin" do
        expect(Zenflow).to receive(:Log).with("Tracking the feature/test-tracking branch against origin/feature/test-tracking")
        expect(Zenflow::Shell).to receive(:run).with("git branch --set-upstream-to=origin/feature/test-tracking feature/test-tracking")
        Zenflow::Branch.track('feature/test-tracking')
      end
    end
  end
  ...
  describe "self.tag" do
    ...
    context "when name and description are not specified" do
      it "asks for name and description and then creates a tag" do
        expect(Zenflow).to receive(:Log).with("Tagging the release")
        expect(Zenflow).to receive(:Ask).with('Name of the tag:', :required => true).and_return('v0.1.3')
        expect(Zenflow).to receive(:Ask).with('Tag message:', :required => true).and_return('this tag is even more amazing')
  ...
  describe "self.delete_remote" do
    ...
    context "when a remote is configured" do
      ...
      it 'pushes to the configured remote' do
        ...
        expect(Zenflow::Shell).to receive(:run).with("git branch -r | grep some-remote/feature/test-remote-removal && git push some-remote :feature/test-remote-removal || echo ''")
    ...
    context "when a remote is not configured" do
      it 'pushes to the origin' do
        ...
        expect(Zenflow::Shell).to receive(:run).with("git branch -r | grep origin/feature/test-remote-removal && git push origin :feature/test-remote-removal || echo ''")
        Zenflow::Branch.delete_remote('feature/test-remote-removal')
      end
    end
    ...
    context "when a backup remote is configured" do
      ...
      it "pushes to the primary remote and then pushes to the backup remote" do
        ...
        expect(Zenflow::Shell).to receive(:run).with("git branch -r | grep backup-remote/feature/test-remote-removal && git push backup-remote :feature/test-remote-removal || echo ''")
        Zenflow::Branch.delete_remote('feature/test-remote-removal')
      end
    end
  end
